...
// AUTOSCROLL
  ...
  $('.header-submenu__item').click(function(){
    ...
    if ($(window).width() <= 480) {    
      $('.header-sub-submenu--' + currentID).css('top', positionTop + currentHeight - scrollOffset);
    };

    $('.header-sub-submenu--' + currentID).perfectScrollbar({
      suppressScrollX: true
    }); 

    $('.header-sub-submenu--' + currentID).fadeToggle(100);
  });
